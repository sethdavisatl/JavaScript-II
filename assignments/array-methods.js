...
let allCaps = runners.map(capRunners => capRunners.first_name.toUpperCase());
console.log(allCaps); 
...
// Problem 1
let numberOfLargeShirts = runners
.filter(shirts => shirts.shirt_size === "L").length;
// .reduce((total, runners) => total + (runners.length), 0);
console.log(numberOfLargeShirts);
// Problem 2
// Initials of contestants
let initialsOfRunners = runners.map(initials => initials.first_name[0] + initials.last_name[0]);
console.log(initialsOfRunners);
// Problem 3
// List of company names sorted alphabetically
companyNames = runners.map(companies => companies.company_name);
let companyNamesSorted = companyNames.sort();
